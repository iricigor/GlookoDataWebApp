...
/**
 * Get chart segment data for coloring the glucose line based on hypo state
 * 
 * Returns an array of segments with start/end time decimals and color information
 * to enable multi-colored line rendering in Recharts.
 * 
 * @param readings - Array of glucose readings sorted by timestamp
 * @param thresholds - Glucose thresholds
 * @returns Array of segments with color information
 */
export interface ChartColorSegment {
  startTimeDecimal: number;
  endTimeDecimal: number;
  color: 'green' | 'lightRed' | 'darkRed';
}

export function getChartColorSegments(
  readings: GlucoseReading[],
  thresholds: GlucoseThresholds
): ChartColorSegment[] {
  ...
  const segments: ChartColorSegment[] = [];
  let currentColor: 'green' | 'lightRed' | 'darkRed';
  let segmentStart = readings[0].timestamp;

  // Determine initial color based on first reading
  const firstValue = readings[0].value;
  if (firstValue < thresholds.veryLow) {
    currentColor = 'darkRed';
  } else if (firstValue < thresholds.low) {
  ...
  for (let i = 1; i < readings.length; i++) {
    const reading = readings[i];
    let newColor: 'green' | 'lightRed' | 'darkRed';
    ...
    if (newColor !== currentColor) {
      // End current segment at previous reading
      const prevReading = readings[i - 1];
      const startDecimal = segmentStart.getHours() + segmentStart.getMinutes() / 60;
      const endDecimal = prevReading.timestamp.getHours() + prevReading.timestamp.getMinutes() / 60;
      
      segments.push({
        startTimeDecimal: startDecimal,
        endTimeDecimal: endDecimal,
        color: currentColor,
      });
  ...
  const startDecimal = segmentStart.getHours() + segmentStart.getMinutes() / 60;
  const endDecimal = lastReading.timestamp.getHours() + lastReading.timestamp.getMinutes() / 60;
