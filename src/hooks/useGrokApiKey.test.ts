/**
 * Tests for useGrokApiKey hook
 */

import { describe, it, expect, beforeEach } from 'vitest';
import { renderHook, act } from '@testing-library/react';
import { useGrokApiKey } from './useGrokApiKey';

describe('useGrokApiKey', () => {
  beforeEach(() => {
  ...
  it('should initialize with empty string when no cookie exists', () => {
    const { result } = renderHook(() => useGrokApiKey());
    expect(result.current.apiKey).toBe('');
  });

  it('should initialize from cookie if it exists', () => {
    // Set a cookie before rendering the hook
    document.cookie = 'glooko-grok-api-key=test-key-123; path=/; SameSite=Strict';
  ...
  it('should save to cookie when setApiKey is called', () => {
    ...
    const apiKeyCookie = cookies.find(c => c.trim().startsWith('glooko-grok-api-key='));
    expect(apiKeyCookie).toBeDefined();
    expect(apiKeyCookie).toContain('saved-key');
  });

  it('should persist across hook re-renders', () => {
    const { result, rerender } = renderHook(() => useGrokApiKey());
  ...
  it('should handle special characters in API key', () => {
    ...
    const { result: result2 } = renderHook(() => useGrokApiKey());
