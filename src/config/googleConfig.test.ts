...
describe('googleConfig', () => {
  ...
  it('should export isGoogleAuthAvailable as true when AUTH_GOOGLE_CLIENT_ID is set', async () => {
    // Mock the environment variable
    vi.stubEnv('AUTH_GOOGLE_CLIENT_ID', 'test-client-id');
  ...
  it('should export isGoogleAuthAvailable as false when AUTH_GOOGLE_CLIENT_ID is not set', async () => {
    // Mock the environment variable as empty
    vi.stubEnv('AUTH_GOOGLE_CLIENT_ID', '');
    ...
    expect(warnSpy).toHaveBeenCalledWith(
      expect.stringContaining('AUTH_GOOGLE_CLIENT_ID environment variable is not configured')
    );
