...
import type { UploadedFile, GlucoseReading, GlucoseUnit } from '../../types';
import {
...
import { CHART_GLUCOSE_THRESHOLDS, MAX_GLUCOSE_VALUES, ROC_INTERVAL_OPTIONS, formatXAxis } from './constants';
import { RoCChartTooltip } from './RoCChartTooltip';
...
export function RoCReport({ selectedFile, glucoseUnit }: RoCReportProps) {
  ...
  const [dayRoCData, setDayRoCData] = useState<import('../../types').RoCDataPoint[]>([]);
  const [dayGlucoseReadings, setDayGlucoseReadings] = useState<GlucoseReading[]>([]);
  const [rocStats, setRocStats] = useState<import('../../types').RoCStats | null>(null);
  ...
  useEffect(() => {
    if (availableDates.length > 0 && allReadings.length > 0) {
      ...
      let rocData: import('../../types').RoCDataPoint[];
          ...
          >
            <ResponsiveContainer width="100%" height="100%">
              <ComposedChart margin={chartMargins} data={chartData}>
                ...
                <RechartsTooltip 
                  content={<RoCChartTooltip glucoseUnit={glucoseUnit} />} 
                />
